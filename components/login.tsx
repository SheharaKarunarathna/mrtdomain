...
export default function LoginModal({ isOpen, onClose, onLoginSuccess }: LoginModalProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 relative max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-800 mb-2">{isSignUp ? 'Create Account' : 'Login'}</h2>
          <p className="text-gray-600 mb-6">
            {isSignUp ? 'Join the mrt.lk community' : 'Sign in to manage your .mrt.lk domains'}
          </p>
          ...
          <form onSubmit={handleEmailAuth} className="mb-6">
            <div className="space-y-4 text-left">
              ...
              {/* Sign-up only: Organization */}
              {isSignUp && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Organization / Purpose (Optional)</label>
                  <input
                    type="text"
                    value={org}
          ...
          {/* OAuth divider & buttons - only show on sign in */}
          {!isSignUp && (
            <>
